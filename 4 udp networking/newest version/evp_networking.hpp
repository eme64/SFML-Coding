...
---- General Ideas about the Net Framework:
 - Game objects -> auto synch.
 - send reliable commands / game inputs
 - game session: lobby and game

 -- concrete:
set server ip -> read from file for now? command line?
game objects
semi-reliable packets
...
struct NServer
{
  ...
  EChat *chat;
  NServer(EChat *chat, ChatInputHandler_t f_chat, DataReqHandler_t f_dataReq);
  ~NServer();
...
struct NClient
{
  ...
  sf::IpAddress recipient =  "localhost"; //"192.168.1.41";//
  ...
  // ------------------------------------- STATE MACHINE end
  NClient(EChat *chat, DataReqHandler_t f_dataReqHandler);
  ~NClient();
  void draw(sf::RenderWindow &window);
  void update();
  void announcePort();
...
// ########################## NServer ########################################
// ########################## NServer ########################################
NServer::NServer(EChat *chat, ChatInputHandler_t f_chat, DataReqHandler_t f_dataReq)
: chat(chat), f_chatInputHandler(f_chat), f_dataReqHandler(f_dataReq)
{
  // bind socket:
...
// ############################## NClient ##############################
// ############################## NClient ##############################
NClient::NClient(EChat *chat, DataReqHandler_t f_dataReqHandler): np_rcv(NPacketRcvHandler(NULL))
{
