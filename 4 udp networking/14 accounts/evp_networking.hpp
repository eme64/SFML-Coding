...
struct NServer
{
  ...
  NServerClient* getClient(uint16_t id);
...
uint16_t NServerClient::id_last = 0;
...
NServerClient* NServer::getClient(uint16_t id)
{
  std::map<int, NServerClient*>::iterator it_nserver;
  it_nserver = map_id.find(id);
  if(it_nserver != map_id.end())
  {
    return it_nserver->second;
  }else{
    return NULL;
  }
